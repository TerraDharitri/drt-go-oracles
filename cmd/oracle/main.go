...
import (
	...
	"github.com/TerraDharitri/drt-go-oracles/config"
	"github.com/TerraDharitri/drt-go-sdk/aggregator"
	"github.com/TerraDharitri/drt-go-sdk/aggregator/api/gin"
	"github.com/TerraDharitri/drt-go-sdk/aggregator/fetchers"
	"github.com/TerraDharitri/drt-go-sdk/aggregator/notifees"
...
func startOracle(ctx *cli.Context, version string) error {
	...
	for key, val := range cfg.DharitriXTokenIDsMappings {
		log.Info("read Dharitrix token IDs mapping", "key", key, "quote", val.Quote, "base", val.Base)
	}
	...
	priceFetchers, err := createPriceFetchers(httpResponseGetter, graphqlResponseGetter, cfg.DharitriXTokenIDsMappings)
	if err != nil {
		return err
	}
	...
	argsNotifee := notifees.ArgsDrtNotifee{
		Proxy:           proxy,
		TxBuilder:       txBuilder,
	...
	drtNotifee, err := notifees.NewDrtNotifee(argsNotifee)
	if err != nil {
		return err
	}
	...
	log.Info("Starting DharitrI Notifee")
...
func createPriceFetchers(httpReponseGetter aggregator.ResponseGetter, graphqlResponseGetter aggregator.GraphqlGetter, tokenIdsMappings map[string]fetchers.DharitriXTokensPair) ([]aggregator.PriceFetcher, error) {
	...
	for exchangeName := range exchanges {
		priceFetcher, err := fetchers.NewPriceFetcher(fetchers.ArgsPriceFetcher{
			FetcherName:        exchangeName,
			ResponseGetter:     httpReponseGetter,
			GraphqlGetter:      graphqlResponseGetter,
			DharitriXTokensMap: tokenIdsMappings,
		})
		if err != nil {
			return nil, err
		}
...
func createAuthClient(proxy workflows.ProxyHandler, cryptoHolder sdkCore.CryptoComponentsHolder, config config.AuthenticationConfig) (authentication.AuthClient, error) {
	args := native.ArgsNativeAuthClient{
		...
		Proxy:                  proxy,
		TokenExpiryInSeconds:   int64(config.TokenExpiryInSeconds),
		Host:                   config.Host,
